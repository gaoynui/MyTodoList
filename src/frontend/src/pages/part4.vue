...
<script>
  export default {
    data () {
      return {
        tableData: [{
          content: '了解Java web开发，了解spring',
          goal: '熟悉Spring MVC,使用Spring Boot搭建Demo'
        }],
        responseData: [],    // 返回的数据
        apiTaskUrl: '/part',      // 后端接口
        whatPart: 'part4'    // 请求的数据类型
      }
    },
    methods: {
            ...
            this.responseData = [
              {
                ...
                name: JSON.parse(JSON.stringify(response.data[3])).name,
                doc: JSON.parse(JSON.stringify(response.data[3])).doc,
                remark: JSON.parse(JSON.stringify(response.data[3])).remark
              }
            ]
          })
          .catch(error => {
